...
class UNET_Dropout_ROI_Context_Enhanced(object):
	...
	def setup_train(self):
		...
		learning_rate = tf.compat.v1.train.exponential_decay(starter_learning_rate, self.global_step, 100000, 0.1, staircase=True)
		...
		if self.num_averaged_gradients == 1:
			...
			for grad, var in grads_and_vars:
				grad_ph = tf.compat.v1.placeholder(grad.dtype, grad.shape)
	...
	def session_TF(self, X_training, Y_training, gender_training, X_testing, Y_testing, gender_testing, mask,
		...
		sess = tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(allow_soft_placement=True, gpu_options=gpu_options))
		...
		if not self.testing_time:
			...
			self.X_train = tf.compat.v1.placeholder(tf.float32, shape=(None, self.size_cube_input, self.size_cube_input,
				self.size_cube_input, self.dim_input), name='X_train')
			self.Y_train = tf.compat.v1.placeholder(tf.float32, shape=(None, self.size_cube_output, self.size_cube_output,
				self.size_cube_output, self.dim_output), name='Y_train')			
			self.X_train_mask = tf.compat.v1.placeholder(tf.bool, shape=(None, self.size_cube_output, self.size_cube_output,
				self.size_cube_output), name='X_train_mask')

			self.X_test = tf.compat.v1.placeholder(tf.float32, shape=(None, self.size_cube_input, self.size_cube_input, self.size_cube_input,
				self.dim_input), name='X_test')
			self.Y_test = tf.compat.v1.placeholder(tf.float32, shape=(None, self.size_cube_output, self.size_cube_output, self.size_cube_output,
				self.dim_output), name='Y_test')
			self.X_test_mask = tf.compat.v1.placeholder(tf.bool, shape=(None, self.size_cube_output, self.size_cube_output, self.size_cube_output),
				name='X_test_mask')

			self.Y_train_global = tf.compat.v1.placeholder(tf.float32, shape=(None,
				self.dim_output), name='Y_test')
			self.Y_test_global = tf.compat.v1.placeholder(tf.float32, shape=(None,
				self.dim_output), name='Y_test')
			...
			self.list_Y_train_context = [tf.compat.v1.placeholder(tf.float32, shape=(None, self.list_size_cube_output_context[_], self.list_size_cube_output_context[_],
				self.list_size_cube_output_context[_], self.dim_output),
				name='Y_train_context_'+str(_)) for _ in range(3)]
	
			self.list_Y_test_context = [tf.compat.v1.placeholder(tf.float32, shape=(None, self.list_size_cube_output_context[_], self.list_size_cube_output_context[_],
				self.list_size_cube_output_context[_], self.dim_output),
				name='Y_test_context_'+str(_)) for _ in range(3)]

			self.list_X_train_mask_context = [tf.compat.v1.placeholder(tf.float32, shape=(None, self.list_size_cube_output_context[_], self.list_size_cube_output_context[_],
				self.list_size_cube_output_context[_]),
				name='X_train_mask_context_'+str(_)) for _ in range(3)]

			self.list_X_test_mask_context = [tf.compat.v1.placeholder(tf.float32, shape=(None, self.list_size_cube_output_context[_], self.list_size_cube_output_context[_],
				self.list_size_cube_output_context[_]),
			...
			if self.import_model:
				...
				v1 = [v for v in tf.compat.v1.global_variables() if "Adam" not in v.name]
				saver_grabber = tf.compat.v1.train.Saver(var_list=v1)
				#saver_grabber = tf.train.Saver()
				print('/data/wenhao/neuroimage/Swin-U-NET-for-LocalBrainAge-prediction-main/saved_model_3D_UNET_Dropout/iteration_'+str(self.iteration_restored))
				saver_grabber.restore(sess,tf.compat.v1.train.latest_checkpoint('/data/wenhao/neuroimage/Swin-U-NET-for-LocalBrainAge-prediction-main/saved_model_3D_UNET_Dropout/iteration_'+str(self.iteration_restored)))
			...
			tf.compat.v1.summary.scalar('re_cost', tf.squeeze(self._loss_op))
			merged = tf.compat.v1.summary.merge_all()
			train_writer = tf.compat.v1.summary.FileWriter('./tensorboard_3D_UNET_Dropout')
			...
			if not self.import_model:
				...
				v1 = [v for v in tf.compat.v1.global_variables() if "Adam" in v.name]
				print(v1)
				sess.run(tf.compat.v1.initialize_variables(var_list = v1))
			...
			for i in range(self.iteration_restored, self.num_iterations - self.iteration_restored):
				...
				for separate_minibatch in tqdm(range(self.num_averaged_gradients), desc=f"Iteration {i}"):
                    ...
                    ##### Codes for different insert position######

 					# if X_training_feed.shape[0] != None and Y_training_feed.shape[0]!= None:
					# 	#print("Entering 3D...")
					# 	X_train = tf.compat.v1.placeholder(tf.float32, shape=(None, 52, 52, 52, 3))
					# 	Y_train = tf.compat.v1.placeholder(tf.float32, shape=(None, 12, 12, 12, 1))
					#
					# 	Block = SwinTransformerBlock3D(dim=3, heads=1, window_size=4)
					# 	#X_training_feed_np = np.array(X_training_feed)
					# 	X_training_feed_processed = Block(X_training_feed)
					# 	Y_training_feed_processed= Block(Y_training_feed)
					#
					# 	X_training_feed_ = sess.run(X_training_feed_processed, feed_dict={X_train: X_training_feed})
					# 	Y_training_feed_ = sess.run(Y_training_feed_processed, feed_dict={Y_train: Y_training_feed})
					#
					# 	X_training_feed = (X_training_feed_ + X_training_feed)*0.5
					# 	Y_training_feed = (Y_training_feed_ + Y_training_feed)*0.5
				...
				print('at iteration '+str(i) + ' Loss : '+str(costul_actual))

		elif self.testing_time:

			X_test = tf.compat.v1.placeholder(tf.float32, shape=(None, self.size_cube_input, self.size_cube_input, self.size_cube_input,
				self.dim_input), name='X_test')
			Y_test = tf.compat.v1.placeholder(tf.float32, shape=(None, self.size_cube_output, self.size_cube_output, self.size_cube_output,
				self.dim_output), name='Y_test')
			Y_test_context = tf.compat.v1.placeholder(tf.float32, shape=(None, self.dim_output),
				name='Y_test_context')		
			X_test_mask = tf.compat.v1.placeholder(tf.bool, shape=(None, self.size_cube_output, self.size_cube_output, self.size_cube_output),
				name='X_test_mask')
			...
			saver_grabber = tf.compat.v1.train.Saver()
			saver_grabber.restore(sess, tf.compat.v1.train.latest_checkpoint('./saved_model_3D_UNET_Dropout/iteration_'+str(self.iteration_restored)))
