	...
	for _, __ in zip(reversed(range(num_encoding_layers)), range(num_encoding_layers)):
		...
		for num_layer_same_scale in range(num_layers_same_scale):
			...
			inputul =propagate_dropout(l=_, num_layer_same_scale = num_layer_same_scale, input_prev_layer = inputul,
				...
				convolution_type='upsampling', deconvolution_shape=intermediate_layers_shape[_])
...
def UNET_network_context_enhanced(inputul, num_encoding_layers, mode, keep_prob,
	...
	for _ in range(num_encoding_layers):
		...
		for num_layer_same_scale in range(num_layers_same_scale):
            ...
            # This is the original code of UNet
			inputul = propagate_dropout(l=_, num_layer_same_scale = num_layer_same_scale, input_prev_layer = inputul, num_stride=1,
				dim_filter=3, num_filters = num_filters[_],
				padding='valid', unet_type=unet_type, mode=mode,
				rate=keep_prob, convolution_type='encoding', deconvolution_shape=None)
		...
		if _!=0:
			...
			with tf.compat.v1.variable_scope('context_level_predition_'+str(_), reuse = tf.compat.v1.AUTO_REUSE):

				list_pred_context.append(tf.compat.v1.layers.conv3d(inputs=inputul,
				...
				list_pred_context_global.append(tf.compat.v1.layers.dense(
	...
	for num_layer_same_scale in range(num_layers_same_scale):
		...
		inputul =propagate_dropout(l=0, num_layer_same_scale = num_layer_same_scale, input_prev_layer = inputul, num_stride=1,
			dim_filter=3, num_filters = num_filters[-1]*2, 
			padding='valid', unet_type=unet_type, mode=mode, rate=keep_prob, convolution_type='context', deconvolution_shape=None)
	...
	with tf.compat.v1.variable_scope('context_level_predition_3', reuse = tf.compat.v1.AUTO_REUSE):
	...
	for _, __ in zip(reversed(range(num_encoding_layers)), range(num_encoding_layers)):
		...
		for num_layer_same_scale in range(num_layers_same_scale):
			...
			inputul =propagate_dropout(l=_, num_layer_same_scale = num_layer_same_scale, input_prev_layer = inputul,
				num_stride=1, dim_filter=3, num_filters = num_filters[_], 
				padding='valid', unet_type=unet_type, mode=mode, rate=keep_prob,
				convolution_type='upsampling', deconvolution_shape=intermediate_layers_shape[_])
		...
		if __ == 0:
			...
			with tf.compat.v1.variable_scope('context_level_predition_4', reuse = tf.compat.v1.AUTO_REUSE):
